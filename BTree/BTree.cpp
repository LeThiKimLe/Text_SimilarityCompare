...
void BTree::insert(word k)
{
	...
	else // If tree is not empty
	{
		...
		/*int i = 0;
		while (i < root->n && root->keys[i].value.compare(k.value) > 0)
		{
			i++;
		}
		if(i!=root->n)
			if(root->keys[i].value.compare(k.value)==0)
			{
				root->keys[i].belong.push_back(k.belong);
				return;
			}*/
...
void BTreeNode::insertNonFull(word k)
{
	...
	if (leaf == true)
	{
		...
		/*if (keys[i].value.compare(k.value) == 0)
		{
			keys[i].belong.push_back(k.belong);
			return;
		}*/
...
{
	cout << "Processing...";
	outfile1.open(nowpath/"Simulate"/"docBTree1.txt", ios::out | ios::trunc);
	outfile2.open(nowpath/"Simulate"/"sosanhBTree1.txt", ios::out | ios::trunc);
